...
#define OBJECTNESS_THRESHOLD 15
...
void InputGen::generate()
{
    ...
    for (int i = 0; i < IMG_SIZE; i++)
    {
        for (int j = 0; j < IMG_SIZE; j++)
        {
            ...
            int val;
            if      (dist == 0) val = 30;          // peak
            else if (dist == 1) val = 10;          // inner ring (steeper drop)
            else                val = rand() % 4;  // background noise (max 3)
...
void Detect::process()
{
    ...
    int best_cell_y   = 0;
    int best_cell_val = 0;
    int cell_sum      = 0;              // sum of all cell maxima for avg background
    ...
    // Compare best cell against the average of ALL cells (including itself).
    // This avoids low scores caused by the Gaussian blob spilling into
    // neighbouring cells, which inflated the old second_cell_val.

    int avg_cell     = cell_sum / num_cells;
    int conf_fp = (best_cell_val > 0)
                  ? ((best_cell_val - avg_cell) * 10000) / best_cell_val
                  : 0;
